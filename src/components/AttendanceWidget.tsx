...
export const AttendanceWidget: React.FC<AttendanceWidgetProps> = ({ user, attendance }) => {
  ...
  const validateLocation = async () => {
    if (!user.assignedLocation) {
       toast.error("No work location assigned. Please contact admin.");
       return null;
    }
    ...
    try {
      ...
      if (distance > (user.assignedLocation.radius || 100)) {
        toast.error(`Location validation failed: You are ${Math.round(distance)}m away from your assigned site.`, { id: 'geo-check' });
        return null;
      }
      ...
      toast.error("Could not verify location. Please ensure GPS is enabled and you have allowed access.", { id: 'geo-check' });
      return null;
    }
  };
    ...
    <div className="bg-[#2A1D0E] border border-[#3D2B1A] rounded-2xl p-6 shadow-xl relative overflow-hidden group">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-[#1C1207] rounded-xl p-4 border border-[#3D2B1A]">
            <p className="text-amber-200/40 text-xs uppercase tracking-widest mb-2 font-mono">Location</p>
            <div className="flex items-center gap-2 text-white font-medium cursor-help" title={user.assignedLocation?.address}>
              <MapPin className="w-4 h-4 text-amber-500" />
              <span className="truncate">{user.assignedLocation?.address || "No Location Set"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8">
          {!isClockedIn ? (
            ...
            >
              ...
              {clockingInProgress ? "Checking Location..." : "Clock In"}
            </button>
          ) : !isClockedOut ? (
            <button
            ...
            >
              <Clock className="w-5 h-5" />
              {clockingInProgress ? "Checking Location..." : "Clock Out"}
            </button>
          ) : (
